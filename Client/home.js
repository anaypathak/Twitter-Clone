console.log("you are at home js")
 ...
 tweet_post.addEventListener("submit",(e)=>{
   ...
   e.preventDefault()
   if(textarea_tweet.value===""){
        alert1.innerHTML= ` <div class="flex items-center justify-center w-12 bg-emerald-500" style="background-color: #ff1100ff;">
      ...
      <div class="px-4 py-2 -mx-3">
          <div class="mx-3">
              <span class="font-semibold  text-black ">Failure!</span>
              <p class="text-sm text-gray-600 dark:text-gray-200">This Area Cannot be empty ! Please fill with some thoughts</p>
          </div>
      </div>`;
   ...
   fetch(`/tweets?tweets_content=${textarea_tweet.value}`).then((response)=>{
...
setTimeout(() => {
  ...
  Array.from(like_btn).forEach((like) => {
   ...
   like.addEventListener("click", (e) =>{
     ...
     prev.setAttribute("fill","#0004fcff")


     let getattr = id_elem.getAttribute("element-id")

      fetch(`/update_data?t_id=${getattr}`).then((response)=>{
